...
=head1 NAME
  ...
  hmmer2htb.pl - convert hmmsearch output to Htb format

=head1 SYNOPSIS
  
  hmmer2htb.pl [-help] [-in input-file] [-out output-file]

  Options:
    -h (--help)   brief help message
    -i (--in)     input file
    -o (--out)    output file
    -s (--seq)    tgt fasta file
    -m (--hmm)    qry hmm file
...
while( my $res = $in->next_result ) {
  while( my $hit = $res->next_hit ) {
    while( my $hsp = $hit->next_hsp ) {
      ...
      exists $hm->{$qid} || die "no size for $qid\n";
      my $qsize = $hm->{$qid};
      my $hsize = $db->length($hid);
      defined $hsize || die "no size for $hid\n";
      my ($qbeg, $qend) = ($hsp->start("query"), $hsp->end("query"));
      my ($hbeg, $hend) = ($hsp->start("hit"), $hsp->end("hit"));
