...
my $align_dir = "E:/Scripts/test/align/align_result/";

sub al2seq() {
    my ($seq, $rCRS, $oneseq) = @_;
    ...
    my $factory = Bio::Tools::Run::StandAloneBlast->new( -program => 'blastn',
    	-outfile => 'bl2seq.out', -F => "F");
    my $bl2seq_report = $factory->bl2seq($seq, $rCRS);
    copy "bl2seq.out", $align_result_file;
...
#        $aln->gap_char("_");
        ...
        print substr($oneseq,0,length($oneseq)-3)."\t".$rCRS_aligned->start()."\t".$rCRS_aligned->end()."\t";
