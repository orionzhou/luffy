...
#DRD
 my @gene_id_arr = (50632,1812..1818);
...
foreach my $gene_id (@gene_id_arr)
{
  ...
  &find_Indel($chr,$start-1000000,$start+1000000);
  print "\n";
}
$dbh->disconnect();
...
sub find_Indel
{
  ...
  my $query  = "select distinct(Locus_ID),Locus_Start,Locus_Stop from CNV "
  	."where Locus_Chr='chr$chr' and VariationType='Indel'"
...
sub fetch_detail_info
{
  ...
  my $query  = "select * from CNV "
  	."where Locus_ID='$locus_ID' and VariationType='Indel'";
  my $sqr = $dbh->prepare($query);
  $sqr->execute();
  while(my $ref = $sqr->fetchrow_hashref())
  {
